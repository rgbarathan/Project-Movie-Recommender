...
def main():
    ...
    if args.evaluate:
        ...
        # Apply calibration if enabled (create new prediction objects with calibrated estimates)
        if calibrator is not None:
            print("Applying calibration to predictions...")
            from surprise import Prediction
            calibrated_preds = []
            for pred in preds:
                calibrated_est = calibrator.predict([[pred.est]])[0]
                calibrated_est = np.clip(calibrated_est, 1.0, 5.0)
                calibrated_preds.append(Prediction(pred.uid, pred.iid, pred.r_ui, calibrated_est, pred.details))
